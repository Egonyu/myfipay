...
- [x] ~~WireGuard management tunnel per router~~ ✅ 2026-07-19 — server side `35ddeff`: wg0 on the droplet (10.77.0.1/16, :51820, UFW open — wg drops unauthenticated packets), migration 008, per-device keypair + tunnel IP lazily provisioned on Setup-script fetch, script gains a RouterOS-7 wg section, `wg-sync.sh` cron reconciles peers, heartbeat pings tunnel IP first (public fallback). **Tunnel verified end-to-end same day**: CHR syd1 handshake + bidirectional ping (<1 ms) over 10.77.0.10 ⇄ 10.77.0.1. Caveat: CHR was configured by hand over the DO web console — interface + keypair created on-router, pubkey exfiltrated via `/tool fetch` against nginx access log, devices row updated to router's key (`wg_private_key` now NULL for that device). **The Setup-script wg section itself is still unexercised** — must be re-verified on a fresh paste before the founder dry-run counts it as self-serve
- [x] ~~RADIUS over the management tunnel~~ ✅ 2026-07-20 — the actual CGNAT enabler, all three legs verified live on CHR syd1: **auth** (hotspot login `256700990009` via 10.77.0.1, radius flag on session), **accounting** (radacct start/stop rows), **DAE/Disconnect** (ACK over tunnel; UDP 3799 from public internet times out — tunnel-only, as intended). Gotcha that cost the session: RouterOS 7.16.2 hotspot NAKs any Disconnect-Request lacking the client's **Framed-IP-Address** with Error-Cause 406 "Unsupported Extension" (misleading name — it's not about session matching; username/session-id/MAC combos all 406 without the IP). Fix: `radius.Session` gains `FramedIP`, populated from `radacct.framedipaddress`. Auth/acct shipped `02aa542`; disconnect fix this commit
...
| RADIUS auth over tunnel: live hotspot login via 10.77.0.1 (session `80000005`, radius flag, radacct open row) after `02aa542` repointed CHR `/radius` | ✅ | 2026-07-19 |
| DAE over tunnel: Disconnect-Request 10.77.0.1→10.77.0.10:3799 **ACK'd**, router log `logged out: radius disconnect`, radacct Stop `Admin-Reset`; same packet to the public IP times out (3799 tunnel-only). Required adding Framed-IP-Address — RouterOS 7.16.2 NAKs 406 without it regardless of username/session-id/MAC | ✅ | 2026-07-20 |
